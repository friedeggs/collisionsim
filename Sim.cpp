...
void Sim::tick_v(Ball &b1, Ball &b2, double dt)
{
    ...
    {    
        ...
        vector<double> v1(2);        // net tangential velocity
        vector<double> v2(2);
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            r1[i] -= strain[i] * force_const / b1.k; // Subtract b1 compression
    
            r2[i] = -b2.r / (b1.r + b2.r) * (relaxed_ds[i]); // Relaxed r Ball 2
            r2[i] -= -strain[i] * force_const / b2.k; // Subtract b2 compression
        }
        
        // Compute the tangential surface velocities due to rotation.
        // v_tan = w * r, and v_tan is perpendicular to r, so...
        ...
        // Find the net tangential component of the ball surface
        // velocity, taking into account both w and v_com
		...
		// ball 1 relative to ball 2
        
        v1[0] -= v2[0];
        v1[1] -= v2[1];        
        
        // If relative velocities are nonzero
        ...
        {                                    
            ...
            f_k[0] = -force[1] * u_k;
            f_k[1] = force[0] * u_k;
            if (dot(f_k, v1) > 0) // Ensure that friction points opposite v1
            {
                f_k[0] *= -1;
                f_k[1] *= -1;        
            }
            
            // Compute the change in COM velocity due to friction
 ...
 * Also advances the angular displacement value.
 */
void Sim::tick_s(Ball &ball, double dt)
{
    ball.theta += dt * ball.w;
    for (int i = 0; i < dim; i++)
